...
import { TrashIcon, MagnifyingGlassIcon, LockClosedIcon } from '@heroicons/react/24/outline';
  ...
  <>
    ...
    <div className="flex flex-col items-center justify-center mt-10 px-5">
      ...
      {isLoading ? (
        ...
        <>
          <div className='grid grid-cols-8 gap-3 w-full max-w-6xl
           bg-white text-color-3 p-4 rounded-xl font-semibold text-center'>
            <p>User Type</p>
            <p>Lastname</p>
            <p>Firstname</p>
            <p>Student ID</p>
          ...
          <div>
            {filteredUsers.slice(0, visibleCount).map((user) => (
              <div key={user._id} className="grid grid-cols-8 gap-4 items-center w-full max-w-6xl text-sm bg-white text-color-2 my-2 p-3 mt-2 rounded-xl shadow-sm hover:shadow-md transition text-center font-semibold">
          ...
          {filteredUsers.length > visibleCount && (
            <ShowMore onClick={() => setVisibleCount(visibleCount + 10)} />
          )}
        </>
      )}
    </div>
